...
predicts entropy instability in every dataset ($r = -0.28$ to $-0.43$). The
top$_1$--top$_2$ affinity gap predicts ratio instability on three of the four
datasets, Davis, Klaeger and Metz ($r = -0.44$, $-0.34$, $-0.27$; all
$p < 0.001$), but not on Anastassiadis ($r = -0.104$, $p = 0.23$), as detailed
below.
...
Within Table~\ref{tab:predictors}, the only pairing that holds on both
pK$_d$ datasets is the ratio family's dependence on the top$_1$--top$_2$ gap,
which compares two affinities directly rather than counting how many pass a
cutoff. As reported above, that dependence also holds on Metz but fails on
Anastassiadis, so it replicates on three of the four datasets and specifically
not on the percent-inhibition readout. The entropy
...
selectivity scores based on sub-threshold values.
...
Commercial kinase selectivity panels vary widely in size, from focused
subpanels of a few dozen kinases to near-kinome panels of several hundred.
On a 50-kinase subpanel, entropy- and S-score-based rankings are reasonably
stable ($r \approx 0.78$--$0.82$), while Gini and ratio rankings are effectively
uncorrelated with what would be obtained on a full kinome panel
($r = -0.09$ for Gini and $r = -0.08$ for ratio at $p = 50$). The practical
consequence is that the smaller the panel, the more the choice of definition
