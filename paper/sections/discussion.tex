...
S-score vs.\ Gini, Table~\ref{tab:correlations}), and it answers a different
question from the $\rho > 0.90$ criterion used in the panel-size analysis
below. The former asks how similarly two distinct definitions rank compounds
on the full panel; the latter asks how similarly the same definition ranks
compounds on a subpanel versus the full panel. The two should not be read as
a single, consistent standard of agreement.
This interchangeability is also a full-panel property: the
...
rather than forming a distinct third cluster, consistent with their shared
dependence on the gap between the top target and its nearest competitors.
...
biochemical binding panels and cellular target-engagement assays, particularly
for inhibitors that bind the inactive, DFG-out kinase conformation, which
cell-free panels built on truncated or active-conformation constructs may not
sample\cite{Binder2026}.
