...
against more inactive kinases --- a serious problem when panels differ in size or
...
$\mathrm{p}K_d$ units.
The score is minus the Shannon entropy of the smoothed, normalized profile ---
...
Gini ($\approx 290$) and the ratio (which does not reach $\rho = 0.90$ even at
the full panel).
It converges slightly slower than the bare entropy because the D1 reliability
gate and the smooth hinge add structure that stabilizes a little later; this is
a modest price for satisfying all four properties, which the entropy does not.
