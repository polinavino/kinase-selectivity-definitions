...
31$ for active compounds (Type~1 instability, above).
...
(worst-case pairwise Spearman $\rho = 0.999$).
...
entropy and the S-score (both $p^* = 110$), and is well ahead of
...
datasets (Spearman $r \approx 1.00$; D3 across choices of the quantile itself is
equally robust, worst-case $\rho \approx 1.00$, and D4 continues to hold exactly),
so more than one construction satisfies the properties.
Generalizing the Shannon functional to any R\'enyi order $\alpha > 0$ -- candidate
score $-H_\alpha(p) = \frac{1}{\alpha-1}\log_2 \sum_i p_i^\alpha$, recovering
Shannon as $\alpha \to 1$ -- gives a second such axis: D1--D4 hold for every
$\alpha > 0$ (the D4 argument that a zero-weight coordinate cannot raise the score
goes through unchanged, since $p_i^\alpha = 0$ whenever $p_i = 0$), while
$\alpha \ne 1$ gives a genuinely different ranking (Spearman agreement with
$\alpha = 1$ is $0.997$--$1.000$ for $\alpha \in \{0.5, 2, 3\}$ on both datasets).
This order also sets how quickly the ranking is recoverable from an incomplete
panel. Repeating the panel-size analysis above for $\alpha \in \{0.01, 0.25, 0.5,
0.75\}$ reaches $p^* = 80$ on Klaeger, ahead of the $\alpha = 1$ candidate's
$p^* = 110$, while $\alpha \in \{2, 3\}$ falls to $p^* = 290$, matching the Gini.
On Metz every order from $0.01$ to $3$ reaches $p^* = 80$, so the gap is specific
to the sparser Klaeger panel.
This speed is not bought by discarding information: the number of distinct scores
and the size of the largest tied group are identical across every order tested,
including $\alpha \to \infty$; the only tied compounds are those with at most one
active kinase, whose normalized profile is a point mass and so has zero entropy
regardless of order. Agreement with the $\alpha = 1$ ranking on the sparsest
compounds ($n_\mathrm{active} < 5$) is as strong as on the rest for $\alpha \leq
0.5$ ($\geq 0.98$ in both subsets, both datasets); it is the opposite,
$\alpha \to \infty$, limit whose agreement with $\alpha = 1$ drops specifically
among the sparsest compounds ($0.69$ on Metz). Lower orders weight the active
kinases more evenly and approach a count of the active set, a quantity a random
subpanel estimates with low variance; the $\alpha \to \infty$ limit is set by the
single largest weight, which a subpanel estimates less reliably. We keep
$\alpha = 1$ as the candidate reported throughout, since Shannon entropy needs no
extra parameter to justify and is the quantity the rest of the paper's checks are
built around; the panel-size dependence on $\alpha$ is reported here as a
property of the family, with a lower order a documented, equally
D1--D4-compliant alternative (full figures in \texttt{family\_panelsize.py} and
...
$\rho = 0.999$).
Between the quantile floor, the R\'enyi order, and $T$, at least three axes of
variation are compatible with D1--D4; establishing whether the properties are
jointly satisfiable only by measures spanning this family, and whether a
uniqueness result analogous to Shannon's holds, is the subject of future work.
