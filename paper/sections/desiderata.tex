\section{Motivated Properties for a Well-Formed Selectivity Measure}
...
\textit{Motivation.}
Compounds with no binding above the $\mathrm{p}K_d > 6$ activity threshold receive
arbitrary selectivity scores, producing rank instability with mean $\sigma = 72.9$
pooled across the four definitions' parameter sweeps, compared with $\sigma = 31.3$
for active compounds ($p < 0.001$, Mann--Whitney $U$ test; Type~1 instability,
above; Figure~\ref{fig:failure_modes}, left).
The pooled figure is not uniform across the four definitions, and we report the
decomposition rather than claim that it holds for each. It is driven by Gini
($\sigma = 86.1$ versus $15.9$) and by entropy ($17.1$ versus $12.5$), is marginal
for the ratio ($24.0$ versus $20.7$), and reverses for the S-score ($18.5$ versus
$24.3$), whose score is exactly $0$ for every one of these compounds and whose
ranks are therefore stable by degeneracy rather than by reliability.
A compound that does not bind cannot be
meaningfully ranked relative to compounds that do.
None of the four definitions assessed here incorporate an explicit reliability
condition. In fact, both
the S-score and selectivity entropy assign scores to compounds with no
sub-micromolar binding, treating sub-threshold values as informative.
...
near-zero ratio score when $k=1$ (the reference off-target is the tied second
target, so the gap is negligible) but a large score when $k=2$ (the reference
skips past the tie). On the 17 Klaeger compounds with a top$_1$--top$_2$ gap
below $0.05$, mean $R(1) = 0.02$ against mean $R(2) = 0.42$.
...
\textit{Motivation.}
As we implement them, the entropy and Gini definitions are parameterized by a
baseline $\beta$ that determines which affinities contribute to the binding
distribution. Our $\beta$ is derived from the published forms' own (implicit) choice of
reference points, $K_a \to 0$ for the entropy and $0\%$ inhibition for the Gini.
...
The D1, D2 and D4 verdicts follow from the functional forms set out above. The
D3 verdicts for the entropy and the Gini are empirical, and apply to the
parameterized forms implemented here rather than to the published definitions,
which leave the reference point implicit.
None of the four definitions assessed satisfies all four properties, but the
candidate measure introduced next does.

\begin{table}[h]
  \centering
  \caption{Satisfaction of the four motivated properties by existing selectivity
...
All three changes are deliberately local, and it follows that on the compounds
both measures score they should rank them almost identically. They do: the
full-panel Spearman correlation between the candidate and the entropy is $+0.999$
on Klaeger and $+0.997$ on Metz. This is expected, since the four properties do not act by
reordering the compounds in the body of the distribution. They act on which
compounds receive a score at all, which is D1 and changes the status of 16 of 222
Klaeger compounds, and on whether the score survives a shift in the analyst's
baseline, which is D3. 
...
\begin{figure}[h]
  ...
  \caption{The two failure modes the candidate measure repairs (Klaeger).
           ...
           definitions' parameter sweeps) for compounds with no binding
           above the activity threshold versus compounds with at least one
           active kinase.
           \emph{Right (D3):} per-compound rank instability under the free
           activity baseline, against the number of active kinases, for the
           entropy and for the candidate. }
...
50 random subpanels per size), the smallest panel reaching mean Spearman correlation
...
well ahead of both Gini ($290$) and ratio ($343$, the full panel).
On Metz it again matches the entropy, but here the S-score converges faster
($p^* = 50$ vs.\ $80$).

Those $p^*$ values score every compound, including ones the D1 gate would refuse
to score, and that matters on a subpanel: a compound whose active kinases fall
outside the drawn subset drops below $\tau^*$ and becomes undefined. On Klaeger
this affects $42\%$ of compounds at a 50-kinase panel, $27\%$ at 110 and $7\%$ at
the full panel; on Metz, which is densely covered, at most $2\%$. Recomputing the
convergence over only the compounds that are D1-defined on both the full panel and
the subpanel, the candidate reaches $\rho > 0.90$ at $p^* = 80$ on Klaeger rather
than $110$, and Metz is unchanged. Two cautions apply. The gated and ungated
$p^*$ are not computed on the same set of compounds, since gating removes the
least-bound ones, which are also the hardest to rank, so part of the improvement
is selection rather than a property of the gate. And the same recomputation moves
the Gini from $p^* = 290$ to $p^* = 80$, which bears on the panel-size
...
The candidate measure above (to which we will refer as the \emph{featured} 
candidate measure) is one member of a family of measures satisfying
...
reason we fix it at this value for our featured candidate measure.
